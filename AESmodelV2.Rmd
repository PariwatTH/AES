...
dat5 <- read.csv("https://raw.githubusercontent.com/PariwatTH/AESmodel/main/item5.csv")
dat5
glimpse(dat5)
...
train_rec_evi5 <- recipe(pevi5~evi5, data=train_evi5) %>%
  ...
  step_tfidf(evi5, prefix = "จากข้อมูล") %>% 
  step_normalize(all_numeric_predictors()) 
...
new_dat5
glimpse(new_dat5)
new_dat5$point5 <- as.factor(new_dat5$point5)

#predict score from answer model
new_ans_dat5 <- new_dat5 %>%
  mutate(ans5_processed = str_extract(text5, "(?<=ตอบ).*?(?=เพราะ)"))
names(new_ans_dat5)[3] <- "ans5" 
new_ans_dat5

pred_ans5 <- mul_lastfit_ans5  %>%
  extract_workflow() %>%
  predict(new_data = new_ans_dat5)

pred_ans5

#predict score from reason model
new_rea_dat5 <- new_dat5 %>%
 mutate(extracted_text = str_extract(text5, "(?<=เพราะ).*?(?=จากข้อมูล)"))
names(new_rea_dat5)[3] <- "rea5"
pred_rea5 <- svm_lastfit_rea5  %>%
  extract_workflow() %>%
  predict(new_data = new_rea_dat5)

pred_rea5

#predict score from evi model
new_evi_dat5 <- new_dat5 %>%
 mutate(extracted_text = str_extract(text5, "(?<=จากข้อมูล).*"))
names(new_evi_dat5)[3] <- "evi5"
pred_evi5 <- svm_lastfit_evi5 %>%
  extract_workflow() %>%
  predict(new_data = new_evi_dat5)

pred_evi5

#ปรับ col ที่ทำนายเป็น pred_ans2 pred_rea2 pred_evi5
names(pred_ans5)[1] <- "pred_ans5"
names(pred_rea5)[1] <- "pred_rea5"
names(pred_evi5)[1] <- "pred_evi5"

#ปรับเป็น numeric
pred_ans5$pred_ans5 <- as.numeric(as.character(pred_ans5$pred_ans5))
pred_rea5$pred_rea5 <- as.numeric(as.character(pred_rea5$pred_rea5))
pred_evi5$pred_evi5 <- as.numeric(as.character(pred_evi5$pred_evi5))
...
full_essay5$point5 <- as.numeric(as.character(full_essay5$point5))
glimpse(full_essay5)
full_essay5
...
###test answer model5
```{r}
new_test_ans5 <- test_ans5 %>% select(ans5,pans5)
pred_ans5 <- mul_lastfit_ans5  %>%
  extract_workflow() %>%
...
###test reason model5
```{r}
new_test_rea5 <- test_rea5 %>% select(rea5,prea5)
pred_rea5 <- svm_lastfit_rea5  %>%
  extract_workflow() %>%
...
```{r}
new_test_evi5 <- test_evi5 %>% select(evi5,pevi5)
pred_evi5 <- svm_lastfit_evi5  %>%
  extract_workflow() %>%
